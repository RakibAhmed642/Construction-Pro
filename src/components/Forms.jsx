...
export const ClientForm = ({ initialData = {}, onClose, onSave }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onSave({
      ...
      imageUrl: imagePreview || `https://ui-avatars.com/api/?name=${encodeURIComponent(name || 'client')}&background=random`,
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="flex flex-col items-center justify-center mb-4">
        <div
          onClick={() => fileInputRef.current?.click()}
          className="relative w-24 h-24 rounded-full border-2 border-dashed border-border flex items-center justify-center cursor-pointer hover:border-primary transition-all overflow-hidden bg-muted group shadow-inner"
        >
          {imagePreview ? (
            <>
              <img src={imagePreview} alt="Preview" className="w-full h-full object-cover" />
      ...
      <Input name="name" label={T.fullName || 'Full Name'} defaultValue={initialData?.name} placeholder="e.g. John Doe" required={true} />
      <div className="grid grid-cols-2 gap-4">
        <Input name="phone" label={T.phoneNumber || 'Phone Number'} defaultValue={initialData?.phone} placeholder="017..." required={false} />
        <Input name="email" type="email" label={T.email || 'Email Address'} defaultValue={initialData?.email} placeholder="john@example.com" required={false} />
      </div>
      <Input name="company" label={T.companyName || 'Company Name'} defaultValue={initialData?.company} placeholder="e.g. ABC Constructions" required={false} />
      <Input name="address" label={T.address || 'Address'} defaultValue={initialData?.address} placeholder="123 Street, City" required={false} />

      <button type="submit" className="w-full bg-primary text-primary-foreground py-3 rounded-xl font-bold hover:shadow-lg hover:shadow-primary/20 hover:-translate-y-0.5 transition-all active:scale-95 mt-6">
        {initialData?.id ? (T.updateClient || 'Update Client') : (T.saveClient || 'Add Client')}
      </button>
    </form>
  );
};
