...
const ProfitPilot = ({ scenarios = [], onAddScenario, onDeleteScenario }) => {
    const { language, translations, theme } = useContext(SettingsContext);
    ...
    useEffect(() => {
        const calculate = () => {
            ...
            let tips = [];
            if (seasonality === 'volatile') tips.push(`🌊 <b>${T.pp_volatility_alert_title}:</b> ${T.pp_volatility_alert_desc}`);
            if (seasonality === 'decay') tips.push(`📉 <b>${T.pp_hype_cycle_title}:</b> ${T.pp_hype_cycle_desc}`);
            if (runway < 6 && runway !== Infinity) tips.push(`⚠️ <b>${T.pp_critical_runway_title}:</b> ${T.pp_critical_runway_desc(runway.toFixed(1))}`);
            if (ratio > 5) tips.push(`🚀 <b>${T.pp_scale_opportunity_title}:</b> ${T.pp_scale_opportunity_desc(ratio.toFixed(1))}`);
            else if (ratio < 3 && ratio > 0) tips.push(`📉 <b>${T.pp_efficiency_warning_title}:</b> ${T.pp_efficiency_warning_desc}`);
            if (lowestCash < 0) tips.push(`💰 <b>${T.pp_financing_needed_title}:</b> ${T.pp_financing_needed_desc}`);
            if (tips.length === 0) tips.push(`✅ ${T.pp_healthy_business}`);

            const s = [], w = [], o = [], t = [];
            if (PM > 50) s.push(T.swot_s_margin); if (roi > 20) s.push(T.swot_s_roi); if (ratio > 3) s.push(T.swot_s_ltv); if (s.length === 0) s.push(T.swot_s_stable);
            if (PM < 20) w.push(T.swot_w_margin); if (runway < 6 && runway !== Infinity) w.push(T.swot_w_runway); if (ratio < 1) w.push(T.swot_w_marketing); if (w.length === 0) w.push(T.swot_w_optimized);
            if (G > 10) o.push(T.swot_o_scaling); if (seasonality === 'seasonal') o.push(T.swot_o_seasonal); if (Marketing < 500) o.push(T.swot_o_ads); if (o.length === 0) o.push(T.swot_o_market);
            if (R > 15) t.push(T.swot_t_risk); if (seasonality === 'volatile') t.push(T.swot_t_revenue); if (Fixed > 5000) t.push(T.swot_t_overhead); if (t.length === 0) t.push(T.swot_t_competition);
            ...
            if (lowestCash < 0 && runway !== Infinity) setAlert({ show: true, msg: `${T.pp_alert_critical} ${runway.toFixed(1)} ${T.pp_alert_months}` });
            else if (lowestCash < 1000) setAlert({ show: true, msg: `${T.pp_alert_warning} (${formatMoney(lowestCash)})` });
            ...
            setOutputs({
                ...
                kpis: {
                    ...
                    m11_payback: avgNet > 0 ? ((Invest-Loan)/avgNet).toFixed(1) + ` ${T.pp_months}` : T.pp_never,
                    m12_runway: runway === Infinity ? '∞' : runway.toFixed(1) + ` ${T.pp_months}`, m13_be: breakEven,
    ...
    useEffect(() => {
        ...
        chartInstances.current.main = new ChartJS(mainChartRef.current, {
            ...
            data: { 
                ...
                datasets: [
                    { label: T.pp_cash_flow_title, data: outputs.charts.cashData, borderColor: '#0ea5e9', backgroundColor: 'rgba(14, 165, 233, 0.2)', fill: true, tension: 0.3, pointRadius: 2, pointBackgroundColor: '#fff' },
                    ...(snapshotData ? [{ label: T.pp_baseline, data: snapshotData, borderColor: themeBaseline, borderDash: [5, 5], fill: false, pointRadius: 0 }] : [])
                ] 
            },
        ...
        chartInstances.current.breakEven = new ChartJS(breakEvenChartRef.current, {
            ...
            data: { labels: outputs.charts.labels, datasets: [
                { label: T.revenue, data: outputs.charts.beRev, borderColor: '#10b981', fill: false, tension: 0.4 },
                { label: T.expenses, data: outputs.charts.beCost, borderColor: '#ef4444', borderDash: [4,4], fill: false, tension: 0.1 }
            ]},
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } }, y: { ticks: { callback: (v) => formatMoney(v) } } } }
        });
        ...
        chartInstances.current.dist = new ChartJS(distChartRef.current, {
            type: 'doughnut',
            data: { labels: [T.pp_cogs_risk, T.pp_opex, T.pp_tax_int, T.pp_profit], datasets: [{ data: [outputs.charts.cogs, outputs.charts.opex, outputs.charts.tax, outputs.charts.profit], backgroundColor: ['#64748b', '#f59e0b', '#94a3b8', '#10b981'], borderWidth: 0 }] },
        ...
        chartInstances.current.radar = new ChartJS(radarChartRef.current, {
            type: 'radar',
            data: { labels: [T.pp_growth, T.pp_profit, T.pp_safety, T.pp_efficiency], datasets: [{ label: T.pp_current, data: [outputs.charts.scoreGrowth, outputs.charts.scoreProfit, outputs.charts.scoreSafety, outputs.charts.scoreEff], borderColor: '#8b5cf6', backgroundColor: 'rgba(139, 92, 246, 0.2)' }] },
    ...
    const resetInputs = () => {
        if(window.confirm(T.pp_confirm_reset)) {
            ...
            showToastMsg(T.pp_reset_complete);
        }
    };

    const handleSaveScenario = () => {
        if (!scenarioName.trim()) { showToastMsg(T.pp_scenario_name_req); return; }
        onAddScenario({ 
            name: scenarioName, 
        ...
        setScenarioName('');
        showToastMsg(T.pp_scenario_saved);
    };
    ...
    const loadScenario = (id) => {
        ...
        if (s) {
            setConfig(s.data);
            showToastMsg(`${T.pp_loaded}: ${s.name}`);
        }
    };
    ...
    const takeSnapshot = () => {
        if (outputs.charts.cashData.length > 0) {
            ...
            showToastMsg(T.pp_snapshot_saved);
        }
    };

    const kpiCards = [
        { id: 'm1_net', label: T.pp_kpi_avg_profit, icon: Wallet, value: formatMoney(outputs.kpis.m1_net), sub: outputs.kpis.m1_sub, subIcon: Activity, colorize: true },
        { id: 'm2_ebitda', label: T.pp_kpi_ebitda, icon: Layers, value: formatMoney(outputs.kpis.m2_ebitda), sub: T.pp_kpi_ebitda_sub },
        { id: 'm3_roi', label: T.pp_kpi_roi, icon: Trophy, value: outputs.kpis.m3_roi, sub: T.pp_kpi_roi_sub, subColor: 'text-emerald-600', border: 'border-b-emerald-500' },
        { id: 'm4_rev', label: T.pp_kpi_total_rev, icon: Coins, value: formatMoney(outputs.kpis.m4_rev), sub: T.pp_kpi_total_rev_sub, subColor: 'text-purple-600' },
        { id: 'm5_margin', label: T.pp_kpi_g_margin, icon: Percent, value: outputs.kpis.m5_margin, sub: T.pp_kpi_g_margin_sub },
        { id: 'm6_cac', label: T.pp_kpi_cac, icon: UserPlus, value: formatMoney(outputs.kpis.m6_cac), sub: T.pp_kpi_cac_sub },
        { id: 'm7_ltv', label: T.pp_kpi_ltv, icon: UserCheck, value: formatMoney(outputs.kpis.m7_ltv), sub: T.pp_kpi_ltv_sub },
        { id: 'm8_ratio', label: T.pp_kpi_ltv_cac, icon: HeartPulse, value: outputs.kpis.m8_ratio, sub: T.pp_kpi_ltv_cac_sub, subColor: 'text-indigo-600', border: 'border-b-indigo-500' },
        { id: 'm9_roas', label: T.pp_kpi_roas, icon: Megaphone, value: outputs.kpis.m9_roas, sub: T.pp_kpi_roas_sub },
        { id: 'm10_opex', label: T.pp_kpi_opex_ratio, icon: FileText, value: outputs.kpis.m10_opex, sub: T.pp_kpi_opex_ratio_sub },
        { id: 'm11_payback', label: T.pp_kpi_payback, icon: Hourglass, value: outputs.kpis.m11_payback, sub: T.pp_kpi_payback_sub },
        { id: 'm12_runway', label: T.pp_kpi_runway, icon: Flame, value: outputs.kpis.m12_runway, sub: T.pp_kpi_runway_sub, subColor: 'text-red-500', border: 'border-b-red-500' },
        { id: 'm13_be', label: T.pp_kpi_breakeven, icon: Flag, value: formatMoney(outputs.kpis.m13_be), sub: T.pp_kpi_breakeven_sub },
        { id: 'm14_floor', label: T.pp_kpi_cash_floor, icon: ArrowDown, value: formatMoney(outputs.kpis.m14_floor), colorize: true, sub: T.pp_kpi_cash_floor_sub },
        { id: 'm15_rpe', label: T.pp_kpi_rev_emp, icon: Users, value: formatMoney(outputs.kpis.m15_rpe), colorize: true, sub: T.pp_kpi_rev_emp_sub },
    ];
    ...
    return (
        <div className={`space-y-8 animate-fade-in pb-12 max-w-[1600px] mx-auto p-4 md:p-6 lg:p-8 ${theme === 'dark' ? 'dark' : ''}`}>
            ...
            {toast.show && (
                <div className="fixed top-20 right-5 z-[100] transform transition-all duration-500 translate-x-0 opacity-100">
                    <div className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 py-4 rounded-xl shadow-2xl flex items-center gap-4 border-l-4 border-emerald-500">
                        ...
                        <div><h6 className="font-bold text-sm">{T.pp_system_notification}</h6><span className="text-xs opacity-90">{toast.msg}</span></div>
                    </div>
                </div>
            )}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                <h2 className="text-3xl font-extrabold tracking-tight text-card-foreground">{T.profitPilot}</h2>
                <p className="text-muted-foreground mt-2 text-sm">{T.pp_subtitle}</p>
                </div>
                ...
                <button onClick={takeSnapshot} className={`flex items-center gap-2 bg-card text-foreground px-4 py-2 rounded-lg text-sm font-semibold transition-all border border-border shadow-sm active:scale-95 group ${snapshotData ? 'border-primary text-primary' : 'hover:bg-accent'}`}>
                    ...
                    <span>{snapshotData ? T.pp_baseline_set : T.pp_snapshot}</span>
                </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
                ...
                <div className="lg:col-span-3 space-y-6 no-print order-2 lg:order-1">
                    <div className="flex items-center gap-2 mb-2 px-1">
                        ...
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">{T.pp_configuration}</h3>
                    </div>
                    ...
                    <div className="glass-card rounded-2xl p-1">
                        <div className="relative">
                            ...
                            <select name="preset" value={config.preset} onChange={handlePresetChange} className="w-full bg-slate-50 dark:bg-slate-900/50 border-0 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-xl focus:ring-2 focus:ring-primary-500 block pl-9 p-3 appearance-none transition-all cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800">
                                <option value="custom">{T.pp_preset_custom}</option>
                                <option value="saas">{T.pp_preset_saas}</option>
                                <option value="ecommerce">{T.pp_preset_ecommerce}</option>
                                <option value="restaurant">{T.pp_preset_restaurant}</option>
                                <option value="agency">{T.pp_preset_agency}</option>
                                <option value="manufacturing">{T.pp_preset_manufacturing}</option>
                            </select>
                    ...
                    <div className="glass-card rounded-2xl overflow-hidden flex flex-col sticky top-24">
                        <div className="p-4 bg-muted/50 border-b border-border flex justify-between items-center">
                            <h3 className="font-bold text-foreground text-xs uppercase tracking-wider">{T.pp_input_parameters}</h3>
                            <button onClick={resetInputs} className="text-[10px] text-slate-400 hover:text-red-500 font-bold uppercase transition-colors flex items-center gap-1"><RotateCcw size={12} /> {T.pp_reset}</button>
                        </div>
                        <div className="p-5 space-y-8 overflow-y-auto custom-scrollbar">
                            <div className="group">
                                <label className="text-[10px] font-bold text-muted-foreground uppercase mb-1.5 flex justify-between">{T.pp_initial_investment} <Wallet className="text-slate-300" /></label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-[10px] font-bold text-muted-foreground uppercase mb-1.5 block">{T.pp_fixed_costs}</label><input type="number" name="fixedCosts" value={config.fixedCosts} onChange={handleInputChange} className="w-full px-3 py-2.5 rounded-xl border border-border bg-background text-sm font-mono font-bold focus:ring-2 focus:ring-primary-500 outline-none transition-all" /></div>
                                <div><label className="text-[10px] font-bold text-muted-foreground uppercase mb-1.5 block">{T.pp_ads_costs}</label><input type="number" name="marketing" value={config.marketing} onChange={handleInputChange} className="w-full px-3 py-2.5 rounded-xl border border-border bg-background text-sm font-mono font-bold focus:ring-2 focus:ring-primary-500 outline-none transition-all" /></div>
                            </div>
                            <div className="space-y-7 pt-2">
                                 <div>
                                    <div className="flex justify-between items-end mb-2"><label className="text-sm font-bold text-foreground">{T.pp_gross_margin}</label><span className="text-xs font-bold text-emerald-600 bg-emerald-100 dark:bg-emerald-900/30 px-2 py-1 rounded-md border border-emerald-200 dark:border-emerald-800">{config.margin}%</span></div>
                                    <input type="range" name="margin" min="0" max="100" value={config.margin} onChange={handleInputChange} className="slider-emerald" />
                                </div>
                                <div>
                                    <div className="flex justify-between items-end mb-2"><label className="text-sm font-bold text-foreground">{T.pp_market_demand}</label><span className="text-xs font-bold text-blue-600 bg-blue-100 dark:bg-blue-900/30 px-2 py-1 rounded-md border border-blue-200 dark:border-blue-800">{config.demand}%</span></div>
                                    <input type="range" name="demand" min="0" max="100" value={config.demand} onChange={handleInputChange} className="slider-blue" />
                                </div>
                                <div>
                                    <div className="flex justify-between items-end mb-2"><label className="text-sm font-bold text-foreground">{T.pp_monthly_growth}</label><span className="text-xs font-bold text-purple-600 bg-purple-100 dark:bg-purple-900/30 px-2 py-1 rounded-md border border-purple-200 dark:border-purple-800">{config.growth}%</span></div>
                                 ...
                                 <div>
                                    <div className="flex justify-between items-end mb-2"><label className="text-sm font-bold text-foreground">{T.pp_risk_factor}</label><span className="text-xs font-bold text-red-500 bg-red-100 dark:bg-red-900/30 px-2 py-1 rounded-md border border-red-200 dark:border-red-800">{config.risk}%</span></div>
                            ...
                            <div className="pt-4 border-t border-border">
                                <button onClick={() => setShowAdvanced(!showAdvanced)} className="text-xs font-bold text-muted-foreground hover:text-primary-500 w-full flex justify-between items-center bg-muted p-3 rounded-lg border border-border transition-all">
                                    <span className="uppercase tracking-wider">{T.pp_advanced_config}</span><ChevronRight size={16} className={`transition-transform duration-300 ${showAdvanced ? 'rotate-90' : ''}`} />
                                </button>
                                {showAdvanced && (
                                    <div className="mt-3 space-y-4 p-4 rounded-xl border border-border bg-background/50">
                                        <div className="bg-card p-3 rounded-lg border border-border"><label className="text-[9px] font-bold text-primary-500 block mb-2 uppercase tracking-wide">{T.pp_seasonality}</label><select name="seasonality" value={config.seasonality} onChange={handleInputChange} className="w-full text-xs font-bold p-2 rounded-lg border border-border bg-background focus:ring-1 focus:ring-primary-500 outline-none"><option value="none">{T.pp_season_linear}</option><option value="seasonal">{T.pp_season_seasonal}</option><option value="volatile">{T.pp_season_volatile}</option><option value="decay">{T.pp_season_decay}</option></select></div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div><label className="text-[9px] font-bold text-muted-foreground block mb-1.5">{T.pp_loan_amount}</label><input type="number" name="loanAmount" value={config.loanAmount} onChange={handleInputChange} className="w-full text-xs font-bold p-2 rounded-lg border border-border bg-card focus:ring-1 focus:ring-primary-500 outline-none" /></div>
                                            <div><label className="text-[9px] font-bold text-muted-foreground block mb-1.5">{T.pp_interest_rate}</label><input type="number" name="interestRate" value={config.interestRate} onChange={handleInputChange} className="w-full text-xs font-bold p-2 rounded-lg border border-border bg-card focus:ring-1 focus:ring-primary-500 outline-none" /></div>
                                            <div><label className="text-[9px] font-bold text-muted-foreground block mb-1.5">{T.pp_tax_rate}</label><input type="number" name="tax" value={config.tax} onChange={handleInputChange} className="w-full text-xs font-bold p-2 rounded-lg border border-border bg-card focus:ring-1 focus:ring-primary-500 outline-none" /></div>
                ...
                <div className="lg:col-span-9 space-y-6 lg:space-y-8 order-1 lg:order-2">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            <div className="lg:col-span-8 flex flex-col h-full border-b lg:border-b-0 lg:border-r border-border lg:pr-8 pb-6 lg:pb-0">
                               <div className="flex justify-between items-center mb-6">
                                    <div><h4 className="text-sm font-bold text-foreground flex items-center gap-2"><div className="w-2 h-6 bg-emerald-500 rounded-full"></div> {T.pp_cash_flow_title}</h4><p className="text-[10px] text-muted-foreground mt-1 pl-3 font-medium">{T.pp_cash_flow_subtitle}</p></div>
                                    <div className="flex gap-4">
                                        <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-emerald-500"></div><span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">{T.pp_current}</span></div>
                                        {snapshotData && <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-slate-400"></div><span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">{T.pp_baseline}</span></div>}
                                    </div>
                                </div>
                            ...
                            <div className="lg:col-span-4 flex flex-col justify-between h-full pl-2">
                               <div className="bg-muted rounded-xl p-5 border border-border">
                                  <h4 className="text-[10px] font-extrabold text-muted-foreground uppercase mb-4 tracking-widest flex items-center gap-2"><Calendar size={14} /> {T.pp_month_1_snapshot}</h4>
                                  <div className="space-y-3 text-sm">
                                      <div className="flex justify-between items-center"><span className="text-muted-foreground text-xs font-semibold">{T.revenue}</span><span className="font-bold text-foreground font-mono">{formatMoney(outputs.rowRev)}</span></div>
                                      <div className="flex justify-between items-center"><span className="text-muted-foreground text-xs font-semibold">{T.pp_cogs_risk}</span><span className="font-semibold text-red-400 font-mono text-xs">-{formatMoney(outputs.rowCOGS)}</span></div>
                                      <div className="flex justify-between items-center"><span className="text-muted-foreground text-xs font-semibold">{T.pp_opex}</span><span className="font-semibold text-orange-400 font-mono text-xs">-{formatMoney(outputs.rowFixedMarketing)}</span></div>
                                      <div className="flex justify-between items-center"><span className="text-muted-foreground text-xs font-semibold">{T.pp_debt_service}</span><span className="font-semibold text-slate-400 font-mono text-xs">-{formatMoney(outputs.rowInterest)}</span></div>
                                      <div className="h-px bg-border my-2"></div>
                                      <div className="flex justify-between items-center"><span className="font-bold text-foreground text-sm">{T.netProfit}</span><span className={`font-bold font-mono text-xl tracking-tight ${outputs.rowNet >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>{formatMoney(outputs.rowNet)}</span></div>
                                  </div>
                               </div>
                               <div className="mt-6 flex-grow flex flex-col justify-end">
                                    <h4 className="text-[10px] font-bold text-muted-foreground uppercase mb-3 tracking-widest text-center">{T.pp_cost_distribution}</h4>
                    ...
                    <div className="glass-card rounded-xl p-0.5 bg-gradient-to-r from-primary/10 to-purple-500/10 border-primary-100 dark:border-primary-900">
                        <div className="bg-background/80 backdrop-blur rounded-[10px] p-5 flex items-start gap-5 relative overflow-hidden">
                            ...
                            <div>
                                <h4 className="text-sm font-bold text-foreground mb-1.5 flex items-center gap-2">{T.pp_ai_analyst} <span className="text-[9px] bg-primary/10 text-primary px-1.5 py-0.5 rounded font-bold uppercase">{T.pp_beta}</span></h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        <div className="glass-card rounded-2xl p-6 flex flex-col relative overflow-hidden">
                            <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2 mb-4"><Activity size={14} className="text-purple-500" /> {T.pp_business_health}</h4>
                        ...
                        <div className="glass-card rounded-2xl p-6 flex flex-col">
                            <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2 mb-5"><Crosshair size={14} className="text-blue-500" /> {T.pp_sensitivity_analysis}</h4>
                            <div className="grid grid-cols-4 gap-2 text-[10px] font-mono text-center flex-grow">
                                <div className="col-start-2 bg-muted p-2 rounded-lg text-muted-foreground font-bold">{T.pp_sens_demand_neg}</div>
                                <div className="bg-slate-100 dark:bg-slate-700 p-2 rounded-lg text-primary font-bold">{T.pp_sens_base}</div>
                                <div className="bg-muted p-2 rounded-lg text-muted-foreground font-bold">{T.pp_sens_demand_pos}</div>
                                <div className="bg-muted p-2 rounded-lg flex items-center justify-center font-bold text-muted-foreground">{T.pp_sens_margin_neg}</div>
                                ...
                                <div className="bg-slate-100 dark:bg-slate-700 p-2 rounded-lg flex items-center justify-center font-bold text-primary">{T.pp_sens_base}</div>
                                ...
                                <div className="bg-muted p-2 rounded-lg flex items-center justify-center font-bold text-muted-foreground">{T.pp_sens_margin_pos}</div>
                        ...
                        <div className="glass-card rounded-2xl overflow-hidden md:col-span-2 xl:col-span-1 flex flex-col h-full">
                            <div className="p-5 bg-muted/50 flex justify-between items-center border-b border-border cursor-pointer group" onClick={() => setShowTable(!showTable)}>
                                <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2"><Table size={14} /> {T.pp_detailed_ledger}</h4>
                            ...
                            <div className={`overflow-hidden transition-all duration-300 flex-grow relative ${showTable ? 'h-[300px]' : 'h-0'}`}>
                                <div className="absolute inset-0 overflow-auto custom-scrollbar">
                                    <table className="w-full text-left">
                                        <thead className="sticky top-0 bg-background z-10 shadow-sm"><tr className="border-b border-border"><th className="px-4 py-3 text-[9px] font-bold text-muted-foreground uppercase">{T.pp_month}</th><th className="px-4 py-3 text-[9px] font-bold text-muted-foreground uppercase text-right">{T.revenue}</th><th className="px-4 py-3 text-[9px] font-bold text-muted-foreground uppercase text-right">{T.expenses}</th><th className="px-4 py-3 text-[9px] font-bold text-muted-foreground uppercase text-right">{T.netProfit}</th><th className="px-4 py-3 text-[9px] font-bold text-muted-foreground uppercase text-right">{T.pp_cash}</th></tr></thead>
                    ...
                    <div className="glass-card rounded-2xl p-6 lg:p-8">
                        <h4 className="text-sm font-bold text-foreground flex items-center gap-2 mb-6"><Scale size={14} className="text-purple-500" /> {T.pp_op_trends}</h4>
                        <div className="chart-wrapper h-[250px]"><canvas ref={breakEvenChartRef}></canvas></div>
                    </div>
                    
                    <div className="space-y-4">
                        <div className="flex items-center gap-3 mb-2 px-1"><div className="w-8 h-8 rounded-lg bg-slate-900 dark:bg-white text-white dark:text-slate-900 flex items-center justify-center shadow-lg"><Gauge size={14} /></div><div><h3 className="text-sm font-bold text-foreground leading-none">{T.pp_mission_control}</h3><span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{T.pp_kpis}</span></div></div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mt-6">
                        <div className="glass-card rounded-2xl p-6">
                            <h4 className="text-sm font-bold text-foreground flex items-center gap-2 mb-4"><Layers size={14} className="text-primary" /> {T.pp_scenario_planner}</h4>
                            <div className="space-y-4">
                                <div className="flex gap-2">
                                    <input type="text" id="scenarioNameInput" value={scenarioName} onChange={e => setScenarioName(e.target.value)} placeholder={T.pp_scenario_placeholder} className="flex-grow bg-muted border border-border rounded-lg px-3 py-2 text-xs font-bold focus:ring-1 focus:ring-primary outline-none" />
                                    <button onClick={handleSaveScenario} className="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg text-xs font-bold transition-colors">{T.save}</button>
                                </div>
                                <div className="bg-muted/50 rounded-xl p-1 border border-border h-[140px] overflow-y-auto custom-scrollbar">
                                    <div className="space-y-1 p-2">
                                        {scenarios.length === 0 && <div className="text-[10px] text-muted-foreground text-center py-4 italic">{T.pp_no_scenarios}</div>}
                        ...
                        <div className="glass-card rounded-2xl p-6">
                            <h4 className="text-sm font-bold text-foreground flex items-center gap-2 mb-4"><Crosshair size={14} className="text-primary" /> {T.pp_swot_analysis}</h4>
                            <div className="grid grid-cols-2 gap-3 h-[180px]">
                                <div className="swot-box bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-800/30"><h5 className="text-[10px] font-extrabold text-emerald-600 uppercase mb-2">{T.pp_strengths}</h5><div className="text-[10px] text-muted-foreground space-y-1">{outputs.swot.s.map((t,i) => <div key={i}>• {t}</div>)}</div></div>
                                <div className="swot-box bg-orange-50/50 dark:bg-orange-900/10 border-orange-100 dark:border-orange-800/30"><h5 className="text-[10px] font-extrabold text-orange-600 uppercase mb-2">{T.pp_weaknesses}</h5><div className="text-[10px] text-muted-foreground space-y-1">{outputs.swot.w.map((t,i) => <div key={i}>• {t}</div>)}</div></div>
                                <div className="swot-box bg-blue-50/50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-800/30"><h5 className="text-[10px] font-extrabold text-blue-600 uppercase mb-2">{T.pp_opportunities}</h5><div className="text-[10px] text-muted-foreground space-y-1">{outputs.swot.o.map((t,i) => <div key={i}>• {t}</div>)}</div></div>
                                <div className="swot-box bg-red-50/50 dark:bg-red-900/10 border-red-100 dark:border-red-800/30"><h5 className="text-[10px] font-extrabold text-red-600 uppercase mb-2">{T.pp_threats}</h5><div className="text-[10px] text-muted-foreground space-y-1">{outputs.swot.t.map((t,i) => <div key={i}>• {t}</div>)}</div></div>
                            </div>
                        </div>
                    </div>
                </div>
