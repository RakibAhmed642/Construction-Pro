...
const TaskEditDrawer = ({ task, onClose, onSave, T }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end overflow-hidden">
      {/* Absolute Full Screen Overlay with Blur */}
      <div 
      ...
      <div className="relative z-10 h-full w-full max-w-sm bg-card border-l border-border shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out">
        <div className="p-5 border-b border-border flex justify-between items-center bg-muted/50">
          <h3 className="text-lg font-bold text-card-foreground">{task && task.id ? T.editTask : T.addTask}</h3>
        ...
        <div className="p-6 flex-1 overflow-y-auto custom-scrollbar space-y-8">
          <div className="space-y-3">
            <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">{T.taskName}</label>
            <input 
              type="text" 
          ...
          <div className="space-y-3">
            <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">{T.priority}</label>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end overflow-hidden">
      ...
      <div className="relative z-10 h-full w-full max-w-sm bg-card border-l border-border shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        <div className="p-5 border-b border-border flex justify-between items-center bg-muted/50">
          <h3 className="text-lg font-bold text-card-foreground">{milestone.id ? T.editMilestone : T.addMilestone}</h3>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-accent text-muted-foreground transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-6 flex-1 overflow-y-auto custom-scrollbar space-y-6">
          <div>
            <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">{T.milestoneName}</label>
            <input 
              type="text" 
          ...
          <div>
            <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">{T.deadline}</label>
            <input 
              type="date" 
          ...
          <div className="flex items-center gap-3 bg-muted p-3 rounded-xl">
            ...
            />
            <label htmlFor="milestoneCompleted" className="text-sm font-medium text-foreground">{T.completed}</label>
          </div>
        </div>
...
const AdvancedTimeline = ({ projects, onProjectSelect, onProjectEdit, onProjectAdd, selectedProject, T, language, clients }) => {
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden flex flex-col h-[700px]">
            <div className="p-4 border-b border-border flex justify-between items-center bg-muted/50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-bold text-card-foreground leading-tight">{T.projectTimeline}</h3>
                ...
                <div className="flex flex-col min-w-max">
                    ...
                    <div className="sticky top-0 z-40 bg-card border-b border-border shadow-sm">
                        <div className="flex">
                            ...
                            >
                                <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{T.projectName}</span>
                            </div>
                ...
                <div className="bg-card rounded-xl border border-border shadow-sm p-6 flex flex-col h-full">
                    ...
                    <div className="bg-muted/50 rounded-xl p-5 border border-border mb-6">
                        <div className="flex items-center justify-between mb-3">
                            <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{T.timeline}</span>
                            {isDelayed && (
                                <span className="flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-100 px-2 py-0.5 rounded-full">
                                    <AlertCircle size={10} /> {T.delayed}
                                </span>
                            )}
                        </div>
                        <div className="flex items-center gap-3">
                ...
                <div className="bg-card rounded-xl border border-border shadow-sm p-6 flex flex-col h-[400px] lg:h-auto">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center gap-2">
                            ...
                            <h4 className="font-bold text-card-foreground">{T.tasks}</h4>
                        </div>
                        <div className="flex items-center gap-3">
                ...
                <div className="bg-card rounded-xl border border-border shadow-sm p-6 flex flex-col h-[400px] lg:h-auto">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center gap-2">
                            ...
                            <h4 className="font-bold text-card-foreground">{T.milestones}</h4>
                        </div>
                        <div className="flex items-center gap-3">
                            <button 
                                        ...
                                        <div className="p-3 rounded-xl border transition-all bg-card border-border">
                                            <div className="flex justify-between items-start">
                                                ...
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    <button onClick={() => handleDeleteMilestone(milestone.id)} className="p-1.5 hover:bg-accent rounded-md text-muted-foreground hover:text-destructive transition-colors"><Trash2 size={14} /></button>
                                                </div>
                                            </div>
...
export default function Projects({ projects = [], clients = [], onAdd, onUpdate, onDelete, onNavigate, onOpenModal, documents = [], onAddDocument, onDeleteDocument, ...restProps }) {
    ...
    const handleTaskDelete = (taskId) => {
        if (!selectedProject || !window.confirm(T.confirmDelete)) return;
    ...
    return (
        <div className="space-y-8 p-1 sm:p-2 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <div className="flex items-center gap-3">
                        <h2 className="text-3xl font-extrabold tracking-tight text-card-foreground">{T.projects}</h2>
                        <button 
                    ...
                    <p className="text-muted-foreground mt-2 text-sm">{T.projectManageSubtitle}</p>
                </div>
                ...
                <div className="grid grid-cols-2 lg:flex gap-4">
                     <div className="bg-card px-5 py-3 rounded-xl border border-border shadow-sm flex items-center gap-3">
                         ...
                         <div><p className="text-xs text-muted-foreground font-bold uppercase">{T.totalProjects}</p><p className="text-xl font-bold">{stats.total}</p></div>
                     </div>
                     <div className="bg-card px-5 py-3 rounded-xl border border-border shadow-sm flex items-center gap-3">
                         <div className="p-2 bg-orange-100 text-orange-600 rounded-lg"><Clock size={20}/></div>
                         <div><p className="text-xs text-muted-foreground font-bold uppercase">{T.ongoingProjects}</p><p className="text-xl font-bold">{stats.ongoing}</p></div>
                     </div>
                     <div className="bg-card px-5 py-3 rounded-xl border border-border shadow-sm flex items-center gap-3">
                         <div className="p-2 bg-slate-100 text-slate-600 rounded-lg"><PauseCircle size={20}/></div>
                         <div><p className="text-xs text-muted-foreground font-bold uppercase">{T.onHoldProjects}</p><p className="text-xl font-bold">{stats.onHold}</p></div>
                     </div>
                     <div className="bg-card px-5 py-3 rounded-xl border border-border shadow-sm flex items-center gap-3">
                         <div className="p-2 bg-emerald-100 text-emerald-600 rounded-lg"><CheckCircle2 size={20}/></div>
                         <div><p className="text-xs text-muted-foreground font-bold uppercase">{T.completedProjects}</p><p className="text-xl font-bold">{stats.completed}</p></div>
                     </div>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-card p-2 rounded-2xl border border-border shadow-sm sticky top-2 z-40">
                <div className="flex bg-muted p-1 rounded-xl w-full sm:w-auto">
                    ...
                    >
                        <LayoutGrid size={16}/> {T.scheduler}
                    </button>
                    <button 
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-muted/50 text-muted-foreground text-xs uppercase tracking-wider border-b border-border">
                                        <th className="p-5 font-bold">{T.projectName}</th>
                                        <th className="p-5 font-bold">{T.client}</th>
                                        <th className="p-5 font-bold">{T.location}</th>
                                        <th className="p-5 font-bold">{T.budget}</th>
                                            ...
                                            <tr key={project.id} onClick={() => setModalProject(project)} className="hover:bg-accent group cursor-pointer">
                                                ...
                                                <td className="p-5 text-right">
                                                    <div className="flex items-center justify-end gap-1">
                                                        ...
                                                        <button onClick={(e) => { e.stopPropagation(); onOpenModal('edit_project', project); }} className="p-2 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded-lg transition-colors" title={T.edit}>
                                                            <Edit size={16} />
                                                        </button>
                                                        <button onClick={(e) => { e.stopPropagation(); onDelete(project.id); }} className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-colors" title={T.delete}>
                                                            <Trash2 size={16} />
                                                        </button>
                                                    </div>
                                                </td>
                                        ...
                                        <tr>
                                            <td colSpan="6" className="p-16 text-center text-muted-foreground">
                                                <div className="flex flex-col items-center justify-center">
                                                    ...
                                                    <p className="font-medium text-lg">{T.noProjectsFound}</p>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
            ...
            {docsProject && (
                <Modal title={T.projectDocuments} onClose={() => setDocsProject(null)} size="max-w-lg">
                    <div className="space-y-6">
