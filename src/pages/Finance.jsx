...
const ProjectFinancialDetailView = ({ project, transactions, onBack, T }) => {
    ...
    const financialSummary = useMemo(() => {
        const income = projectTransactions.filter(t => t.type === 'income').reduce((sum, t) => sum + Number(t.amount), 0);
        const expense = projectTransactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + Number(t.amount), 0);
        ...
        <div className="space-y-6 animate-fade-in-up">
            <div className="bg-card rounded-xl p-6 shadow-sm border border-border">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-sm text-muted-foreground">{T.projectReport}</p>
                        <h2 className="text-2xl font-bold text-card-foreground">{project.name}</h2>
                        <p className="text-sm text-muted-foreground">{T.client}: {project.clientName}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
                    <div className="flex justify-between items-center mb-2">
                        <h3 className="font-semibold text-card-foreground">{T.totalRevenue}</h3>
                 ...
                 <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
                    <div className="flex justify-between items-center mb-2">
                        <h3 className="font-semibold text-card-foreground">{T.totalExpenses}</h3>
                 ...
                 <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
                    <div className="flex justify-between items-center mb-2">
                        <h3 className="font-semibold text-card-foreground">{T.netBalance}</h3>
            ...
            <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
                <h3 className="font-bold text-card-foreground p-6 border-b border-border">{T.transactionsForProject}</h3>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-border">
                            {projectTransactions.length > 0 ? projectTransactions.map(t => (
                                ...
                                <tr><td colSpan="4" className="p-12 text-center text-muted-foreground">{T.noTransactionsForProject}</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
...
const Finance = ({ transactions, projects, onDelete, onOpenModal }) => {
    ...
    const projectFinancials = useMemo(() => {
      return projects.map(proj => {
        ...
        const income = projTrans.filter(t => t.type === 'income').reduce((sum, t) => sum + Number(t.amount), 0);
        const expense = projTrans.filter(t => t.type === 'expense').reduce((sum, t) => sum + Number(t.amount), 0);
        return {
          ...proj,
          totalIncome: income,
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
             <h2 className="text-2xl font-bold text-foreground">{T.financialHub}</h2>
        ...
        {activeView === 'transactions' ? (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-muted/50 text-muted-foreground text-sm border-b border-border">
                      ...
                      <th className="p-4 font-semibold">{T.projectContext}</th>
                  ...
                  <tbody className="divide-y divide-border">
                        ...
                        <tr key={t.id} className="hover:bg-accent/50 transition-colors group">
                          ...
                          <td className="p-4 text-sm text-muted-foreground">
                             {linkedProject ? (
                               ...
                               <span className="text-muted-foreground italic">{T.general}</span>
                             )}
                          </td>
                    ...
                    {filteredTransactions.length === 0 && <tr><td colSpan="7" className="p-8 text-center text-muted-foreground">{T.noTransactionsFound}</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              >
                 <div className="flex justify-between items-start mb-4">
                   ...
                   <span className={`px-2 py-1 text-xs rounded-full font-medium ${proj.netProfit >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {proj.netProfit >= 0 ? T.profitable : T.loss}
                   </span>
                 </div>
                 
                 <div className="space-y-3">
                   <div className="flex justify-between items-center p-2 bg-green-500/10 rounded-lg">
                      <span className="text-sm text-green-700 flex items-center gap-2"><ArrowUpRight size={14}/> {T.income}</span>
                   ...
                   <div className="flex justify-between items-center p-2 bg-red-500/10 rounded-lg">
                      <span className="text-sm text-red-700 flex items-center gap-2"><ArrowDownLeft size={14}/> {T.expense}</span>
            ...
            {projectFinancials.length === 0 && <div className="col-span-full text-center py-10 text-muted-foreground">{T.noProjectsToReport}</div>}
          </div>
        )}
      </div>
    );
