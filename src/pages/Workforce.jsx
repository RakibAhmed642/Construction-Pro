...
const PayslipModal = ({ payslip, onClose, T = {} }) => {
    ...
    const monthYear = new Date(payslip.month).toLocaleString(T.locale || 'en-US', { month: 'long', year: 'numeric' });
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-hidden">
            ...
            <div className="relative z-10 bg-card rounded-2xl w-full max-w-lg shadow-2xl transform transition-all scale-100 flex flex-col animate-fade-in">
                <div className="flex justify-between items-center p-6 border-b border-border">
                    <div>
                        <h3 className="text-xl font-bold text-card-foreground">{T.payslipTitle ? T.payslipTitle(monthYear) : `Payslip for ${monthYear}`}</h3>
                ...
                <div className="p-6" id="payslip-content">
                    <div className="grid grid-cols-2 gap-8">
                        {/* Earnings */}
                        <div>
                            <h4 className="font-bold text-lg text-green-600 mb-2 border-b-2 border-green-200 pb-1">{T.earnings || 'Earnings'}</h4>
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between"><span className="text-muted-foreground">{T.baseSalary || 'Base Salary'}</span><span className="font-medium text-foreground">৳{payslip.baseSalary.toLocaleString()}</span></div>
                                <div className="flex justify-between"><span className="text-muted-foreground">{T.overtimePay || 'Overtime Pay'} ({payslip.overtimeHours} hrs)</span><span className="font-medium text-foreground">৳{payslip.overtimePay.toLocaleString()}</span></div>
                            </div>
                        ...
                        {/* Deductions */}
                        <div>
                            <h4 className="font-bold text-lg text-red-600 mb-2 border-b-2 border-red-200 pb-1">{T.deductions || 'Deductions'}</h4>
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between"><span className="text-muted-foreground">{T.absences || 'Absences'} ({payslip.absentDays} days)</span><span className="font-medium text-foreground">- ৳{payslip.absenceDeductions.toLocaleString()}</span></div>
                            </div>
                            <div className="flex justify-between mt-3 pt-3 border-t border-border font-bold">
                    ...
                    <div className="mt-8 bg-muted p-4 rounded-lg text-center">
                        <p className="text-sm font-medium text-muted-foreground">{T.netPayable || 'Net Payable'}</p>
                ...
                <div className="p-4 bg-muted/50 border-t border-border text-right">
                    <button onClick={() => window.print()} className="bg-foreground text-background px-4 py-2 rounded-lg flex items-center gap-2 transition-colors shadow-sm ml-auto"><Printer size={16} /> {T.print || 'Print'}</button>
                </div>
            </div>
        </div>
    );
...
// প্রপসগুলোতে ডিফল্ট ভ্যালু দেওয়া হয়েছে
const Workforce = ({
    attendance = [],
    payrollData = [],
    equipment = [],
    ...
    const tabs = [
        { id: 'attendance', label: T.attendance || 'Attendance', icon: UserCheck },
        { id: 'payroll', label: T.payroll || 'Payroll', icon: Wallet },
        { id: 'equipment', label: T.equipment || 'Equipment', icon: Truck },
        { id: 'materialManagement', label: T.materialManagement || 'Materials', icon: Archive },
    ];

    return (
        <div className="space-y-8">
            <div>
                <h2 className="text-3xl font-bold text-foreground">{T.workforce || 'Workforce'}</h2>
                <p className="text-muted-foreground mt-1">{T.workforceSubtitle || 'Manage your workforce, payroll, and assets.'}</p>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard title={T.employees || 'Employees'} value={employees.length} icon={Users} color="bg-blue-500" />
                <StatCard title={T.totalAssets || 'Total Assets'} value={equipment.length} icon={Wrench} color="bg-orange-500" />
                ...
                <div className="flex space-x-2">
                    {tabs.map(tab => (
                        <button
                            ...
                            className={`flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-all ${activeTab === tab.id ? 'bg-primary/10 text-primary shadow-sm' : 'text-muted-foreground hover:bg-accent/50'
                                }`}
                        >
