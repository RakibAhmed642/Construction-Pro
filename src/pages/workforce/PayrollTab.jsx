...
const PayrollTab = ({ payrollData = [], employees = [], attendance = [], T = {}, onOpenPayslip }) => {
    ...
    const payrollRates = context.payrollRates || {};
    const overtimeMultiplier = context.overtimeMultiplier || 1.5;
    ...
    const payrollSummary = useMemo(() => {
     ...
     const StatusBadge = ({ status }) => {
        ...
        return <span className={`px-2 py-1 text-xs font-semibold rounded-full ${styles[status] || 'bg-gray-100 text-gray-700'}`}>{T[`status${status}`] || status}</span>;
    };
    ...
    const generateCSV = () => {
        ...
        const rows = payrollSummary.map(p => [
            p.name,
            p.role,
            currentMonth.toLocaleString(T.locale || 'en-US', { month: 'long', year: 'numeric'}),
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <h3 className="text-xl font-bold text-card-foreground">{T.payrollOverview || 'Payroll Overview'}</h3>
                 <div className="flex items-center gap-3">
                    <div className="flex items-center bg-card border border-border rounded-lg">
                        ...
                        <span className="px-4 text-sm font-semibold w-36 text-center">{currentMonth.toLocaleString(T.locale || 'en-US', { month: 'long', year: 'numeric' })}</span>
                        <button onClick={() => handleMonthChange(1)} className="p-2 border-l border-border hover:bg-accent"><ChevronRight size={16}/></button>
                    </div>
                    <button onClick={generateCSV} className="bg-primary text-primary-foreground px-4 py-2 rounded-lg flex items-center gap-2"><Download size={16}/> {T.generatePayslips || 'Generate Payslips'}</button>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-muted text-muted-foreground">
                            <tr>
                                <th className="p-4 font-medium">{T.employee || 'Employee'}</th>
                                <th className="p-4 font-medium text-right">{T.baseSalary || 'Base Salary'}</th>
                                <th className="p-4 font-medium text-right">{T.overtimePay || 'Overtime'}</th>
                                <th className="p-4 font-medium text-right">{T.deductions || 'Deductions'}</th>
                                <th className="p-4 font-medium text-right">{T.netSalary || 'Net Salary'}</th>
                                <th className="p-4 font-medium">{T.paymentStatus || 'Status'}</th>
                        ...
                        <tbody className="divide-y divide-border">
                            ...
                            {payrollSummary.length === 0 && (
                                <tr><td colSpan="7" className="p-8 text-center text-muted-foreground">No payroll data available.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
