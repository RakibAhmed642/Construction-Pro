...
const Dashboard = ({ 
    ...
    useEffect(() => {
        const getGreeting = () => {
            ...
            if (hour >= 12 && hour < 16) return T.goodAfternoon?.(name) || `Good Afternoon, ${name}`;
            if (hour >= 16 && hour < 18) return T.goodLateAfternoon?.(name) || `Good Late Afternoon, ${name}`;
            if (hour >= 18 && hour < 21) return T.goodEvening?.(name) || `Good Evening, ${name}`;
            if (hour >= 21 && hour < 24) return T.goodNight?.(name) || `Good Night, ${name}`;
                ...
                <div className="relative z-10 flex flex-col h-full justify-between">
                    <div>
                        <div className="flex justify-between items-start mb-2">
                            <h3 className="font-bold text-xl tracking-tight">{T.safetyStatus || 'Safety Status'}</h3>
                            <CheckCircle className="text-emerald-100" size={24} />
                        </div>
                        <p className="text-emerald-50 opacity-90 text-sm font-medium">{T.safetyRecord || 'Site Safety Record'}</p>
                    </div>

                    <div className="py-6 flex items-baseline gap-2">
                        <span className="text-6xl font-bold tracking-tighter drop-shadow-sm">124</span>
                        <span className="text-sm font-medium text-emerald-100 uppercase tracking-widest">{T.daysWithoutIncident || 'Days Without Incident'}</span>
                    </div>
