...
const Settings = () => {
    ...
    const { 
        ...
        overtimeMultiplier = 1.5, setOvertimeMultiplier = () => {},
        roles = [], setRoles = () => {},
        saveToDb = () => {},
        translations = {} 
    } = context;
    ...
    return (
        <div className="space-y-8 animate-fade-in-up max-w-4xl mx-auto pb-20">
            <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold text-foreground">{T.settings || 'Settings'}</h2>
                <button 
                    onClick={handleSave}
            ...
            {showSuccess && (
                <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-xl flex items-center gap-3 text-emerald-600 animate-in fade-in slide-in-from-top-2">
                    <CheckCircle size={20} />
                    <span className="font-semibold">{T.settingsSaved || 'Settings saved successfully!'}</span>
                </div>
            )}

            <div className="bg-card p-8 rounded-2xl border border-border shadow-sm space-y-8">
                <h3 className="text-xl font-bold text-foreground flex items-center gap-2">
                    <User size={20} className="text-primary" /> {T.profile || 'Profile'}
                </h3>
                ...
                <div className="flex flex-col md:flex-row items-center gap-10">
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                        <div className="space-y-2">
                            <label className="text-sm font-semibold text-muted-foreground">{T.userName || 'User Name'}</label>
                            <input 
                                type="text" 
                                value={tempUserName} 
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-semibold text-muted-foreground">{T.businessName || 'Business Name'}</label>
                            <input 
                                type="text" 
            ...
            <div className="bg-card p-8 rounded-2xl border border-border shadow-sm space-y-6">
                <div className="flex justify-between items-center flex-wrap gap-4">
                    <h3 className="text-xl font-bold text-foreground flex items-center gap-2">
                        <Wallet size={20} className="text-emerald-500" /> {T.payrollSettings || 'Payroll Settings'}
                    </h3>
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3 bg-muted/50 p-2 rounded-xl border border-border">
                            <label className="text-xs font-bold text-muted-foreground uppercase">{T.overtimeMultiplier || 'Overtime Multiplier'}</label>
                            <input 
                                type="number" 
                                step="0.1"
                                value={tempOT}
                        ...
                        <button 
                            ...
                            title={T.addNewRole || 'Add New Role'}
                        >
                        ...
                        {tempRoles?.map(role => (
                            <div key={role} className="space-y-1.5 p-4 bg-muted/30 rounded-xl border border-border/50 relative group/role">
                                <div className="flex justify-between items-start gap-2">
                                    ...
                                    {(!DEFAULT_ROLES || !DEFAULT_ROLES.includes(role)) && (
                                        <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-card p-8 rounded-2xl border border-border shadow-sm">
                    <h3 className="text-xl font-bold text-foreground mb-6 flex items-center gap-2">
                        <Sun size={20} className="text-amber-500" /> {T.theme || 'Theme'}
                    </h3>
                    <div className="flex gap-4">
                        ...
                        >
                            <Sun size={20} />
                            <span className="font-bold">{T.light || 'Light'}</span>
                        </button>
                        <button
                        ...
                        >
                            <Moon size={20} />
                            <span className="font-bold">{T.dark || 'Dark'}</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="bg-card p-8 rounded-2xl border border-border shadow-sm">
                    <h3 className="text-xl font-bold text-foreground mb-6 flex items-center gap-2">
                        <Languages size={20} className="text-blue-500" /> {T.language || 'Language'}
                    </h3>
                    <div className="flex gap-4">
                        <button
                        ...
                        >
                            <span className="font-bold">{T.english || 'English'}</span>
                        </button>
                        <button
                        ...
                        >
                            <span className="font-bold">{T.bengali || 'Bengali'}</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            {/* Add Role Modal */}
            {isAddRoleModalOpen && (
                <Modal title={T.addNewRole || 'Add New Role'} onClose={() => setIsAddRoleModalOpen(false)}>
                    <form onSubmit={handleAddRole} className="space-y-4">
                        <div className="space-y-2">
                            <label className="text-sm font-semibold text-muted-foreground">{T.roleCategory || 'Role / Category'}</label>
                            <div className="relative">
