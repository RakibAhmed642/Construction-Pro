          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex gap-3 w-full md:w-auto">
               <div className="text-right hidden md:block px-4 border-r border-border">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide">{T.lifetimePaid}</p>
               ...
               <button onClick={() => onPay(employee)} className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg flex items-center justify-center gap-2 transition-colors shadow-lg shadow-green-600/20 w-full md:w-auto">
                 <Banknote size={20} /><span>{T.newPayment}</span>
               </button>
            </div>
          </div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-3 space-y-4">
            <div className="bg-card p-4 rounded-xl border border-border shadow-sm">
               <h3 className="text-sm font-semibold text-muted-foreground mb-3 flex items-center gap-2"><Calendar size={16} /> {T.selectFinancialYear}</h3>
            ...
            <div className="bg-gradient-to-br from-primary to-primary/80 p-6 rounded-xl shadow-lg text-primary-foreground">
              <p className="text-primary-foreground/70 text-sm font-medium mb-1">{T.monthlyTotalFor(months[selectedMonth], selectedYear)}</p>
              <h3 className="text-3xl font-bold mb-4">৳{stats.monthlyTotal.toLocaleString()}</h3>
              <div className="pt-4 border-t border-primary-foreground/20">
                 <div className="flex justify-between items-center text-sm"><span className="text-primary-foreground/70">{T.yearlyTotal}</span><span className="font-semibold">৳{stats.yearlyTotal.toLocaleString()}</span></div>
              </div>
            </div>

            <div className="bg-card p-6 rounded-xl border border-border shadow-sm space-y-4">
                <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                    <UserCheck size={16} className="text-blue-500" /> {T.attendance}
                </h3>
                <div className="space-y-3">
                    <div className="flex justify-between items-center">
                        <span className="text-sm text-muted-foreground">{T.monthlyAttendance}</span>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-border">
                        <span className="text-sm text-muted-foreground">{T.yearlyAttendance}</span>
                        <div className="flex items-baseline gap-1">
                 ...
                 <table className="w-full text-left border-collapse">
                   <thead>
                     <tr className="bg-muted/50 border-b border-border">
                       <th className="p-4 text-xs font-bold text-muted-foreground uppercase tracking-wider w-32">{T.date}</th>
                       <th className="p-4 text-xs font-bold text-muted-foreground uppercase tracking-wider">{T.description}</th>
                       <th className="p-4 text-xs font-bold text-muted-foreground uppercase tracking-wider w-32">{T.category}</th>
                       <th className="p-4 text-xs font-bold text-muted-foreground uppercase tracking-wider text-right w-40">{T.amount}</th>
                     </tr>
                   </thead>
                   <tbody className="divide-y divide-border">
                     {filteredTransactions.length > 0 ? (
                       ...
                       <tr><td colSpan="4" className="p-12 text-center"><div className="flex flex-col items-center justify-center text-muted-foreground"><Banknote size={48} className="mb-3 opacity-20" /><p className="text-sm font-medium">{T.noPaymentsFound(months[selectedMonth], selectedYear)}</p></div></td></tr>
                     )}
                   </tbody>
                   {filteredTransactions.length > 0 && <tfoot><tr className="bg-muted/50 border-t border-border"><td colSpan="3" className="p-4 text-right text-sm font-bold text-muted-foreground">{T.totalFor(months[selectedMonth])}</td><td className="p-4 text-right text-sm font-bold text-primary">৳{stats.monthlyTotal.toLocaleString()}</td></tr></tfoot>}
                 </table>
               </div>
            </div>
          </div>
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <h2 className="text-2xl font-bold text-foreground">{T.employeesDirectory}</h2>
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            <button onClick={handleExportCSV} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors shadow-sm"><Download size={20} /><span>{T.exportReport}</span></button>
            <button onClick={() => onOpenModal('employees')} className="bg-primary hover:bg-primary/90 text-primary-foreground px-4 py-2 rounded-lg flex items-center gap-2 transition-colors shadow-sm"><Plus size={20} /><span>{T.addEmployee}</span></button>
          </div>
        </div>
          ...
          {filteredEmployees.map(emp => {
            ...
            return (
                ...
                <div className="space-y-3 text-sm border-t border-border pt-4">
                    <div className="flex justify-between items-center"><span className="text-muted-foreground flex items-center gap-2"><Briefcase size={14}/> {T.projects}</span><span className="font-medium text-foreground truncate max-w-[120px]">{project?.name || T.unassigned}</span></div>
                    <div className="flex justify-between items-center"><span className="text-muted-foreground flex items-center gap-2"><Phone size={14}/> {T.contact}</span><span className="font-medium text-foreground">{emp.phone}</span></div>
                    <div className="bg-muted p-3 rounded-lg flex justify-between items-center mt-2 group-hover:bg-primary/10 transition-colors">
                    <div><p className="text-xs text-muted-foreground">{T.totalPaid}</p><p className="font-bold text-foreground">৳{Number(emp.totalPaid || 0).toLocaleString()}</p></div>
                    <button onClick={(e) => handlePay(e, emp)} className="bg-card border border-border text-foreground text-xs px-3 py-1.5 rounded-md hover:bg-green-600 hover:text-white hover:border-green-600 transition-colors flex items-center gap-1 shadow-sm"><Banknote size={14} /> {T.pay}</button>
                    </div>
                </div>
                </div>
            );
          })}
          {filteredEmployees.length === 0 && <div className="col-span-full bg-card p-12 rounded-xl border-2 border-dashed border-border text-center text-muted-foreground"><Briefcase size={48} className="mx-auto mb-4 text-muted" /><p>{searchQuery ? T.noEmployeeSearchResults : T.noEmployeesFound}</p></div>}
        </div>
      </div>
    );
};
